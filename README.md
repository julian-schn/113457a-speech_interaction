...
3. Start the wakeword app with transcription enabled:
   ```bash
   python eliza-speech.py --playback_device hw:2,0 --vad_threshold 0.5 --model_path models/hey_mycroft_v0.1.onnx --transcribe_url http://127.0.0.1:8080/inference 
   ```
...
python app-offline.py --mode offline --audio_dir data\neg_wavs --model_path .\models\hey_mycroft_v0.1.onnx --threshold 0.5 --release_ratio 0.9 --hop_ms 40 --capture_seconds 0 --print_triggers

or in a sweep for different threshholds results:

foreach ($t in 0.1,0.5,0.8) {  python app-offline.py --mode offline --audio_dir data\test_mycroft --model_path .\models\hey_mycroft_v0.1.onnx --threshold $t --release_ratio 0.9 --hop_ms 50 --capture_seconds 0 --print_triggers}
